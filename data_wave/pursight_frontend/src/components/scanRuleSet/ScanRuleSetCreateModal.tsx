...
interface ScanRuleSetCreateModalProps {
  open: boolean;
  onClose: () => void;
  dataSourceId?: number;
  onSuccess?: () => void;
...
const ScanRuleSetCreateModal: React.FC<ScanRuleSetCreateModalProps> = ({
  open,
  onClose,
  dataSourceId,
  onSuccess,
}) => {
  const { createScanRuleSet, isCreatingScanRuleSet, validatePatterns } = useScanRuleSets();
  ...
  const formik = useFormik({
    initialValues: {
      name: '',
      description: '',
      is_global: dataSourceId ? false : true,
    ...
    onSubmit: async (values) => {
      try {
        setError(null);
        await createScanRuleSet(values);
        onSuccess?.();
        onClose();
      } catch (err: any) {
        setError(err.response?.data?.detail || 'Failed to create scan rule set');
      }
    },
  });

  // Set data source ID if provided as prop
  useEffect(() => {
    if (dataSourceId) {
      formik.setFieldValue('data_source_id', dataSourceId);
      formik.setFieldValue('is_global', false);
    }
  }, [dataSourceId]);
    ...
    <Dialog open={open} onClose={onClose} maxWidth="md" fullWidth>
      <DialogTitle>
        <Box display="flex" justifyContent="space-between" alignItems="center">
          <Typography variant="h6">Create Scan Rule Set</Typography>
      ...
      <form onSubmit={formik.handleSubmit}>
        ...
        <DialogActions sx={{ px: 3, py: 2 }}>
          ...
          <Button
            ...
            variant="contained"
            color="primary"
            disabled={isCreatingScanRuleSet}
            startIcon={isCreatingScanRuleSet ? <CircularProgress size={20} /> : null}
          >
            {isCreatingScanRuleSet ? 'Creating...' : 'Create Rule Set'}
          </Button>
        </DialogActions>
      </form>
    </Dialog>
...
export default ScanRuleSetCreateModal;