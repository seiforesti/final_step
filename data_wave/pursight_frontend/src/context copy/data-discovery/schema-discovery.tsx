...
export function SchemaDiscovery({ 
  ...
  const discoverSchema = async () => {
    ...
    try {
      ...
      const progressTracker = setupProgressTracking(dataSourceId, (progress: any) => {
        console.log(`📊 Progress update received:`, progress)
        if (progress) {
          // Handle different progress event formats
          const percentage = progress.percentage || progress.progress_percentage || 0
          const status = progress.status || progress.current_stage || progress.step || ''
          
          console.log(`📈 Processing progress: percentage=${percentage}, status=${status}`)
          
          if (typeof percentage === 'number') {
            const progressValue = Math.round(percentage * 100)
            console.log(`📊 Setting progress to: ${progressValue}%`)
            setDiscoveryProgress(progressValue)
          }
          ...
          if (status) {
            ...
            if (progress.step === 'schemas_listed' && typeof progress.schemas_total === 'number') {
              ...
              label = `Schema ${progress.schema} (${Math.min(percentage * 100, 99)}%): ${progress.table} (${progress.tables_in_schema} total)`
            ...
            console.log(`📝 Setting status to: ${label}`)
      ...
      const firstLevelIds = treeNodes.map((node: SchemaNode) => node.id)
