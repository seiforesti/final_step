...
export class IntegrationValidator {
  ...
  private async validateSystemIntegrations(): Promise<void> {
    ...
    try {
      const wsUrl = `${(typeof window !== 'undefined' && (window as any).ENV?.NEXT_PUBLIC_WS_URL) || 'ws://localhost:8000'}/ws/scan-rules`;
  ...
  private getExpectedAPIEndpoints(componentId: string): Array<{ name: string; url: string }> {
    const baseUrl = (typeof window !== 'undefined' && (window as any).ENV?.NEXT_PUBLIC_API_BASE_URL) || '/api/v1';
