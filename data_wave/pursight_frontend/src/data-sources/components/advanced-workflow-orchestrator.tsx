...
export function AdvancedWorkflowOrchestrator({
  ...
  const [workflowHistory, setWorkflowHistory] = useState<
    Array<{ id: string; name: string; status: string; completedAt: string }>
  >([])
    ...
    async (workflowId: string) => {
      ...
      try {
        ...
        while (stepQueue.length > 0) {
          const readySteps = stepQueue.filter((step) => step.dependencies.every((dep) => completedSteps.has(dep)))
  ...
  return (
    <div className={`bg-zinc-950 text-zinc-100 border border-zinc-800 rounded-lg p-4 space-y-4 ${className}`}>
      ...
      <div className="flex items-center justify-between border-b border-zinc-800 pb-3">
        <div>
          <h2 className="text-lg font-semibold text-zinc-100 font-mono">Advanced Workflow Orchestrator</h2>
        ...
        <div className="flex items-center space-x-2">
          <Badge
            ...
            className={`text-xs px-2 py-1 ${
              isRunning ? "bg-red-900/50 text-red-300 border-red-800" : "bg-zinc-800 text-zinc-300 border-zinc-700"
            }`}
          >
      ...
      {activeWorkflows.size > 0 && (
        <Alert className="bg-zinc-900 border-zinc-700 text-zinc-300">
          ...
          <AlertDescription className="text-xs">{activeWorkflows.size} workflow(s) currently running</AlertDescription>
        </Alert>
      )}
              ...
              >
                <CardHeader className="pb-2 p-3">
                  <div className="flex items-center justify-between">
                    <CardTitle className="text-sm font-mono text-zinc-100">{workflow.name}</CardTitle>
                  ...
                  <p className="text-xs text-zinc-400 leading-relaxed">{workflow.description}</p>
                </CardHeader>
                <CardContent className="p-3 pt-0">
                  <div className="space-y-2">
                    <div className="flex items-center justify-between text-xs text-zinc-400">
                      <span>Steps:</span>
                      <span className="font-mono">{workflow.steps.length}</span>
                    </div>
                    <div className="flex items-center justify-between text-xs text-zinc-400">
                      <span>Duration:</span>
                      <span className="font-mono">
                        {Math.round(workflow.steps.reduce((acc, step) => acc + step.estimatedDuration, 0) / 1000)}s
                      </span>
                    </div>
                    <Button
                      ...
                      disabled={isRunning}
                      size="sm"
                      className="w-full h-7 text-xs bg-zinc-800 hover:bg-zinc-700 text-zinc-100 border border-zinc-700"
                    >
        ...
        <TabsContent value="active" className="space-y-3">
          {activeWorkflows.size === 0 ? (
            <div className="text-center py-6">
              <Workflow className="h-8 w-8 mx-auto mb-2 text-zinc-600" />
              <p className="text-xs text-zinc-500">No active workflows</p>
            </div>
          ) : (
            <div className="space-y-3">
              {Array.from(activeWorkflows.entries()).map(([workflowId, workflow]) => (
                <Card key={workflowId} className="bg-zinc-900 border-zinc-800">
                  <CardHeader className="p-3 pb-2">
                    <CardTitle className="flex items-center space-x-2 text-sm">
                      {getCategoryIcon(workflow.category)}
                      <span className="font-mono text-zinc-100">{workflow.name}</span>
                      <Badge
                        className={`text-xs px-1 py-0 ${
                          workflow.priority === "critical"
                            ? "bg-red-900/50 text-red-300 border-red-800"
                  ...
                  <CardContent className="p-3 pt-0">
                    <div className="space-y-2">
                      {workflow.steps.map((step, index) => (
                        <div
                          key={step.id}
                          className="flex items-center space-x-2 p-2 bg-zinc-800/50 rounded border border-zinc-800"
                        >
                          {getStepStatusIcon(step.status)}
                          <div className="flex-1">
                            <div className="flex items-center justify-between">
                              <span className="text-xs font-mono text-zinc-200">{step.name}</span>
                              <span className="text-xs text-zinc-500 font-mono">
                                {step.actualDuration ? `${step.actualDuration}ms` : `${step.estimatedDuration}ms`}
                              </span>
                            </div>
                            {step.status === "running" && <Progress value={50} className="mt-1 h-1 bg-zinc-800" />}
                          </div>
                        </div>
                      ))}
                    </div>
        ...
        <TabsContent value="history" className="space-y-3">
          {workflowHistory.length === 0 ? (
            <div className="text-center py-6">
              ...
              <p className="text-xs text-zinc-500">No workflow execution history</p>
            </div>
          ) : (
            <div className="space-y-2">
              {workflowHistory.map((entry) => (
                <Card key={entry.id} className="bg-zinc-900 border-zinc-800">
                  <CardContent className="p-3">
                    <div className="flex items-center justify-between">
                      <div>
                        <p className="text-sm font-mono text-zinc-200">{entry.name}</p>
