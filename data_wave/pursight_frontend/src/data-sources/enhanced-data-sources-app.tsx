...
function EnhancedDataSourcesAppContent({ className, initialConfig }: EnhancedDataSourcesAppProps) {
  ...
  return (
    <TooltipProvider>
      ...
      >
        {/* Cursor-style Dark Theme Layout */}
        <div className={`min-h-screen bg-[#1e1e1e] text-[#cccccc] ${className}`}>
          
          {/* Main Layout - No Header, Just Sidebar + Content */}
          <div className="flex h-screen">
            
            {/* Cursor-style Minimal Sidebar */}
            <aside className={`bg-[#252526] border-r border-[#3c3c3c] flex flex-col transition-all duration-300 ${sidebarCollapsed ? 'w-12' : 'w-60'}`}>
              
              {/* Top Section - Logo and Toggle */}
              <div className="flex items-center justify-between p-3 border-b border-[#3c3c3c]">
                {!sidebarCollapsed && (
                  <div className="flex items-center space-x-2">
                    <div className="w-6 h-6 bg-[#007acc] rounded-sm flex items-center justify-center">
                      <Database className="h-4 w-4 text-white" />
                    </div>
                    <span className="text-[#cccccc] text-sm font-medium">Data Sources</span>
                  </div>
                )}
                <Button
                  variant="ghost"
                  ...
                  className="h-8 w-8 text-[#cccccc] hover:bg-[#2d2d30] hover:text-white"
                >
                  <Menu className="h-4 w-4" />
                </Button>
              </div>

              {/* Centralized Search - Cursor Style */}
              <div className="p-3 border-b border-[#3c3c3c]">
                <div className="relative">
                  <Search className="absolute left-2 top-1/2 transform -translate-y-1/2 h-4 w-4 text-[#858585]" />
                  <Input
                    placeholder={sidebarCollapsed ? "⌘K" : "Search everything... (⌘K)"}
                    value={searchQuery}
                    onChange={(e) => setSearchQuery(e.target.value)}
                    onFocus={() => setCommandPaletteOpen(true)}
                    className="pl-8 bg-[#3c3c3c] border-[#464647] text-[#cccccc] placeholder-[#858585] focus:bg-[#1e1e1e] focus:border-[#007acc] h-8 text-sm"
                  />
                  <Button
                    variant="ghost"
                    size="sm"
                    className="absolute right-1 top-1/2 transform -translate-y-1/2 h-6 px-2 text-xs text-[#858585] hover:text-[#cccccc]"
                  ...
                  <div key={categoryKey} className="mb-6">
                    {!sidebarCollapsed && (
                      <div className="px-2 py-1 mb-2">
                        <h3 className="text-[#858585] text-xs font-medium uppercase tracking-wider">
                          {category.label}
                        </h3>
                      </div>
                    )}
                    ...
                    <div className="space-y-1">
                      {category.items.map((item: any) => (
                        <Tooltip key={item.id} delayDuration={sidebarCollapsed ? 0 : 1000}>
                          <TooltipTrigger asChild>
                            ...
                            >
                              <item.icon className={`h-4 w-4 ${sidebarCollapsed ? '' : 'mr-2'} ${
                                activeView === item.id ? 'text-white' : 'text-[#858585]'
                              }`} />
                              {!sidebarCollapsed && (
                          ...
                          {sidebarCollapsed && (
                            <TooltipContent side="right" className="bg-[#1e1e1e] border-[#3c3c3c] text-[#cccccc]">
                              {item.label}
                            </TooltipContent>
                          )}
                        </Tooltip>
              ...
              {/* Bottom Section - User and Settings */}
                  ...
                  {!sidebarCollapsed && (
                    <div className="flex items-center space-x-2 flex-1">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="text-[#cccccc] text-sm truncate">{currentUser?.name || 'User'}</div>
                        <div className="text-[#858585] text-xs truncate">{currentUser?.roles?.[0]?.toString() || 'Member'}</div>
                      </div>
                    </div>
                  )}
                  <Button
                    variant="ghost"
                    size="icon"
                    className="h-8 w-8 text-[#858585] hover:bg-[#2d2d30] hover:text-white"
                  >
            ...
            <main className="flex-1 flex flex-col bg-[#1e1e1e] overflow-hidden">
              ...
              <div className="h-10 bg-[#2d2d30] border-b border-[#3c3c3c] flex items-center justify-between px-4">
                <div className="flex items-center space-x-2">
                  ...
                  {selectedDataSource && (
                    <>
                      ...
                      <span className="text-[#858585] text-sm">{selectedDataSource.name}</span>
                    </>
                  )}
                </div>
                ...
                <div className="flex items-center space-x-1">
                  <Button variant="ghost" size="sm" className="h-7 text-[#858585] hover:text-white hover:bg-[#37373d]">
          ...
          {commandPaletteOpen && (
            <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-sm">
              <div className="fixed top-20 left-1/2 transform -translate-x-1/2 w-full max-w-2xl">
                <div className="bg-[#252526] border border-[#3c3c3c] rounded-lg shadow-2xl">
                  <div className="p-4">
                    <div className="relative">
                      <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 h-4 w-4 text-[#858585]" />
                      <Input
                        placeholder="Search data sources, commands, and actions..."
                        value={searchQuery}
                        onChange={(e) => setSearchQuery(e.target.value)}
                        className="pl-10 bg-[#1e1e1e] border-[#3c3c3c] text-[#cccccc] placeholder-[#858585] focus:border-[#007acc] h-12"
                        autoFocus
                      />
                    </div>
                  </div>
                  
                  <div className="border-t border-[#3c3c3c] max-h-96 overflow-y-auto">
                    <div className="p-2">
                      {/* Recent Actions */}
                      <div className="mb-4">
                        <div className="px-2 py-1 text-[#858585] text-xs font-medium uppercase tracking-wider">
                          Recent Actions
                        </div>
                        <div className="space-y-1">
                          <div className="flex items-center space-x-3 p-2 hover:bg-[#37373d] rounded cursor-pointer">
                            <Database className="h-4 w-4 text-[#007acc]" />
                            <span className="text-[#cccccc] text-sm">View Data Sources</span>
                          </div>
                          <div className="flex items-center space-x-3 p-2 hover:bg-[#37373d] rounded cursor-pointer">
                            <BarChart3 className="h-4 w-4 text-[#007acc]" />
                            <span className="text-[#cccccc] text-sm">Analytics Dashboard</span>
                          </div>
                        </div>
                      </div>
                      ...
                      <div>
                        ...
                        <div className="space-y-1">
                          <div className="flex items-center space-x-3 p-2 hover:bg-[#37373d] rounded cursor-pointer">
                            <Plus className="h-4 w-4 text-[#007acc]" />
                            <span className="text-[#cccccc] text-sm">Add Data Source</span>
                          </div>
                          <div className="flex items-center space-x-3 p-2 hover:bg-[#37373d] rounded cursor-pointer">
                            <Workflow className="h-4 w-4 text-[#007acc]" />
                            <span className="text-[#cccccc] text-sm">Create Workflow</span>
                          </div>
                        </div>
                      </div>
                    </div>
