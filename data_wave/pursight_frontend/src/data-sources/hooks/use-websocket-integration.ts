...
export function useWebSocketIntegration(config: WebSocketConfig = {}) {
  ...
  const defaultConfig: Required<WebSocketConfig> = {
    url: (typeof window !== 'undefined' && (window as any).ENV?.NEXT_PUBLIC_WS_URL) || 'ws://localhost:8000/ws/data-sources',
    protocols: ['data-governance-v1'],
    reconnectInterval: 5000,
    maxReconnectAttempts: 10,
    heartbeatInterval: 30000,
    enableCompression: true,
    enableDebugLogging: (typeof window !== 'undefined' && (window as any).ENV?.NODE_ENV) === 'development',
    bufferMaxSize: 1000,
