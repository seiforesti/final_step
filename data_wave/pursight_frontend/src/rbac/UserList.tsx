      ...
      render: (is_active: boolean, record: User) => (
        is_active ? (
          <Button
            icon={<StopOutlined />}
            danger
            onClick={() => deactivateUserMutation.mutate(record.id)}
          ...
          <Button
            icon={<CheckOutlined />}
            type="primary"
            onClick={() => activateUserMutation.mutate(record.id)}
      ...
      render: (_: any, record: User) => (
        <Space>
          <Select
            ...
            value={selectedRole[record.id] ?? undefined}
            onChange={(val) => setSelectedRole((prev) => ({ ...prev, [record.id]: val }))}
          ...
          <Button
            ...
            type="primary"
            disabled={assignRoleMutation.isPending || !selectedRole[record.id]}
            onClick={() => {
              if (selectedRole[record.id])
                assignRoleMutation.mutate({
                  userId: record.id,
                  roleId: selectedRole[record.id]!,
                });
            }}
          >
            Assign
