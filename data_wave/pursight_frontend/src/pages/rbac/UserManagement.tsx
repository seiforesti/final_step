...
import { Table, Button, Tag, Select, Space, Typography, message, Modal, Tooltip, Input } from "antd";
import { PlusOutlined, DeleteOutlined, CheckOutlined, StopOutlined, UserAddOutlined, ExclamationCircleOutlined, SearchOutlined } from "@ant-design/icons";
...
const removeRole = async (userId: number, roleId: number) => {
  await axios.post(`/sensitivity-labels/users/${userId}/remove-role`, { role_id: roleId });
};
...
const UserManagement: React.FC = () => {
      ...
      render: (roleNames: string[], record: User) => (
        <Space wrap>
          {roleNames.map((roleName) => {
            ...
            return (
              <Tag color="geekblue" key={roleName} style={{ fontSize: 14, padding: '2px 8px', borderRadius: 8 }}>
                {roleName}
                {roleObj && (
  ...
  return (
    <div style={{ padding: 24, background: '#181c24', minHeight: '100vh' }}>
      <Title level={3} style={{ color: '#fff', marginBottom: 24 }}>User Management (RBAC Admin)</Title>
