...
export interface CrossGroupSearchRequest {
  query: string;
  groups?: string[];
...
export interface CrossGroupSearchResponse {
  results: any[];
  total: number;
  groups: string[];
  facets?: any;
...
export interface DependencyMappingResponse {
  mappingId: UUID;
  dependencies: any[];
  status: string;
}
...
export interface CrossGroupAnalyticsRequest {
  timeRange: { start: ISODateString; end: ISODateString };
  groups?: string[];
  metrics?: string[];
}

export interface CrossGroupAnalyticsResponse {
  metrics: any[];
  insights: any[];
  recommendations: any[];
}
...
export interface CoAuthoringSession {
  ...
  conflict_resolution_strategy: 'merge' | 'lock' | 'branch';
}

export interface StartCoAuthoringSessionRequest {
  document_id: UUID;
  participants?: UUID[];
  conflict_resolution_strategy?: 'merge' | 'lock' | 'branch';
}

export interface CoAuthoringSessionResponse {
  session: CoAuthoringSession;
...
export interface CreateCommentRequest {
  content: string;
  target_id: UUID;
  target_type: 'document' | 'section' | 'line';
  position?: {
    line?: number;
    column?: number;
...
export interface CollaborationAnalyticsRequest {
  ...
  metrics?: ('engagement' | 'productivity' | 'communication' | 'conflicts')[];
}
