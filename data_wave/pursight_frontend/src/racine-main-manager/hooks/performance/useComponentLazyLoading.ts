...
interface ComponentFactory {
  id: string
  factory: () => Promise<any>
  metadata?: Omit<ComponentMetadata, 'id' | 'path'>
}
...
export const useComponentLazyLoading = (options: LazyLoadOptions = {}) => {
  ...
  // Component factory registry for Next.js-compatible lazy loading
  const componentFactoryRef = useRef<Map<string, () => Promise<any>>>(new Map())

  // Register a component factory for lazy loading (Next.js compatible)
  const registerComponentFactory = useCallback((id: string, factory: () => Promise<any>, metadata?: Omit<ComponentMetadata, 'id' | 'path'>) => {
    componentFactoryRef.current.set(id, factory)
    
    // Also register in legacy registry for compatibility
    const fullMetadata: ComponentMetadata = {
      id,
      path: `<factory:${id}>`, // Placeholder path for factory-based components
      ...metadata
    }
    componentRegistry.current.set(id, fullMetadata)
  }, [])

  // Register a component for lazy loading (legacy method)
  ...
  // Register multiple component factories
  const registerComponentFactories = useCallback((factories: ComponentFactory[]) => {
    factories.forEach(({ id, factory, metadata }) => {
      registerComponentFactory(id, factory, metadata)
    })
  ...
  const loadComponent = useCallback(async (componentId: string): Promise<LoadResult> => {
    ...
    try {
      ...
      // Check if component has a factory function (Next.js compatible)
      const factory = componentFactoryRef.current.get(componentId)
      if (factory) {
        const module = await factory()
        component = module.default || module[componentId]
        
        if (!component) {
          throw new Error(`Component ${componentId} not found in factory module`)
        }
      } else {
        // No factory registered - components must use registerComponentFactory for Next.js compatibility
        throw new Error(`Component ${componentId} must be registered using registerComponentFactory() for Next.js compatibility. Dynamic imports with variable paths are not supported.`)
      }
