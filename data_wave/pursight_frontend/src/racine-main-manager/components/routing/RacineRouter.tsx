...
// Browser-safe router shims
const useRouter = () => ({
  push: (path: string) => { if (typeof window !== 'undefined') window.history.pushState({}, '', path) },
  replace: (path: string) => { if (typeof window !== 'undefined') window.history.replaceState({}, '', path) },
});
const usePathname = () => (typeof window !== 'undefined' ? window.location.pathname : '/');
const useSearchParams = () => (typeof window !== 'undefined' ? new URLSearchParams(window.location.search) : new URLSearchParams());
