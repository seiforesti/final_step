...
export class ComponentInterconnectionManager {
  ...
  private eventStore: ComponentEvent[];
  ...
  registerComponent(componentId: string, component: any, rbacContext?: any): void {
    ...
    this.stateStore.set(componentId, component.state || {});
  ...
  unregisterComponent(componentId: string): void {
    ...
    this.stateStore.delete(componentId);
    this.metrics.totalComponents--;
  ...
  emitEvent(event: ComponentEvent): void {
    ...
    // Store event for reactive listeners
   ...
   */
  subscribeToComponent(componentId: string, eventTypes: string[] = []): (callback: (event: ComponentEvent) => void) => () => void {
    return (callback: (event: ComponentEvent) => void) => {
      const handler = (event: ComponentEvent) => {
        if (event.source === componentId && 
   ...
   */
  subscribeToComponentState(componentId: string): (callback: (state: any) => void) => () => void {
    return (callback: (state: any) => void) => {
      const handler = (event: ComponentEvent) => {
        if (event.type === 'state-changed' && event.payload?.componentId === componentId) {
          callback(event.payload.newState);
        }
      };
      ...
      // Return unsubscribe function
      return () => {
        this.eventEmitter.off('state-changed', handler);
      };
    };
  }
  ...
  updateComponentState(componentId: string, newState: any, rbacContext?: any): void {
    const currentState = this.stateStore.get(componentId);
    if (!currentState) {
    ...
    this.stateStore.set(componentId, newState);

    // Emit state change event
    this.emitEvent({
      type: 'state-changed',
      source: componentId,
      payload: { componentId, newState, previousState },
      timestamp: new Date(),
      rbacContext
    });
  }
  ...
  getComponentState(componentId: string): any {
    return this.stateStore.get(componentId) || null;
  }
