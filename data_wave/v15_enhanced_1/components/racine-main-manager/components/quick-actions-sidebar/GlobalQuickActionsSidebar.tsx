...
import { getQuickActionIcon, generateActionId } from "../../utils/quick-action-utils"
import { checkActionPermissions } from "../../utils/security-utils"
import { formatActionTime } from "../../utils/formatting-utils"

// Import constants (already implemented)
import type { SIDEBAR_ANIMATIONS } from "../../constants/quick-action-constants"
...
export const GlobalQuickActionsSidebar: React.FC<GlobalQuickActionsSidebarProps> = ({
  ...
  const getContextActions = useCallback(
    async (ctx: any) => {
      try {
        return (await qaOps?.getContextualActions?.(ctx)) ?? []
      } catch {
        return []
      }
    },
  ...
  const getFavoriteActions = useCallback(async () => {
    try {
      return (await qaOps?.getFavoriteActions?.()) ?? []
    } catch {
      return []
    }
  }, [qaOps])
  ...
  const getRecentActions = useCallback(
    async (limit?: number) => {
      try {
        return (await qaOps?.getRecentActions?.(limit)) ?? []
      } catch {
        return []
      }
    },
  ...
  const executeAction = useCallback(
    async (id: string, params?: any) => {
      try {
        return await qaOps?.executeAction?.(id, params)
      } catch {
  ...
  const trackActionUsage = useCallback(
    (id: string) => {
      try {
        qaOps?.trackActionUsage?.(id)
      } catch {
        /* noop */
      }
    },
  ...
  const getActionAnalytics = useCallback(async () => {
    try {
      return (await qaOps?.getActionAnalytics?.()) ?? {}
    } catch {
      return {}
    }
  }, [qaOps])

  const saveActionPreferences = useCallback(
    async (p: any) => {
      try {
        await qaOps?.saveUserPreferences?.(p)
      } catch {
        /* noop */
      }
    },
  ...
  const canExecuteAction = (...args: any[]) => {
    try {
      return crossGroupOps?.canExecuteAction?.(...args)
    } catch {
      return false
    }
  }
  const getSPAPermissions = (...args: any[]) => {
    try {
      return crossGroupOps?.getSPAPermissions?.(...args)
    } catch {
      return []
    }
  }
  const coordinateAction = async (...args: any[]) => {
    try {
      return await crossGroupOps?.coordinateAction?.(...args)
    } catch {
      /* noop */
    }
  }
  ...
  const getCurrentUser = () => {
    try {
      return userOps?.getCurrentUser?.() ?? userState?.currentUser ?? null
    } catch {
      return null
    }
  }
  const getUserPermissions = () => {
    try {
      return userOps?.getUserPermissions?.() ?? (userState?.userPermissions || [])
    } catch {
      return []
    }
  }
  const checkUserAccess = useCallback(
    (permission: string, perms?: any[]) => {
      try {
    ...
    [userOps, userState?.userPermissions],
  )
  ...
  const trackEvent = useCallback(
    (name: string, payload?: any) => {
      try {
        activityHook?.operations?.trackEvent?.(name, payload)
      } catch {
        /* noop */
      }
    },
    [activityHook?.operations],
  )
  const trackComponentUsage = useCallback(
    (id: string, action: string) => {
      try {
        activityHook?.operations?.trackComponentUsage?.(id, action)
      } catch {
        /* noop */
      }
    },
  ...
  const getUsageAnalytics = useCallback(async () => {
    try {
      return (await activityHook?.operations?.getUsageAnalytics?.()) ?? {}
    } catch {
      return {}
    }
  }, [activityHook?.operations])
  ...
  const getUserPreferences = () => {
    try {
      return prefsOps?.getUserPreferences?.() ?? prefsState?.userPreferences ?? {}
    } catch {
      return {}
    }
  }
  const updateUserPreferences = async (p: any) => {
    try {
      await prefsOps?.updateUserPreferences?.(p)
    } catch {
      /* noop */
    }
  }
  const getSidebarPreferences = () => {
    try {
      return prefsOps?.getSidebarPreferences?.() ?? prefsState?.sidebarPreferences ?? {}
    } catch {
      return {}
    }
  }
  const updateSidebarPreferences = useCallback(
    async (p: any) => {
      try {
        await prefsOps?.updateSidebarPreferences?.(p)
      } catch {
        /* noop */
      }
    },
  ...
  const { getSystemMetrics, optimizePerformance } = useRacineOrchestration()
  ...
  useEffect(() => {
    ...
    const initializeSidebar = async () => {
      try {
        ...
        if (persistLayout && sidebarPreferences.customLayout) {
          if (mounted) setCustomizedLayout(sidebarPreferences.customLayout)
        }

        // Load contextual actions if enabled
        if (enableContextualActions) {
          const contextActions = await getContextActions(currentContext)
          if (mounted) setContextualActions(contextActions)
        }

        // Load recent actions and favorites
        const [recentActions, favoriteActions] = await Promise.all([getRecentActions(20), getFavoriteActions()])

        if (mounted) {
          setActionHistory(recentActions)
        }

        if (enableAnalytics && mounted) {
          try {
            const analytics = await getActionAnalytics()
            if (mounted) setComponentMetrics(analytics)
          } catch (e) {
          ...
          analyticsIntervalRef.current = setInterval(async () => {
            ...
            try {
              ...
              if (mounted) setComponentMetrics((prev) => ({ ...prev, ...metrics }))
            } catch {
              // keep UI responsive
            }
    ...
    currentContext,
    persistLayout,
    sidebarPreferences.customLayout,
    enableContextualActions,
    enableAnalytics,
    getContextActions,
    getRecentActions,
  ...
  const loadComponent = useCallback(
    async (componentId: string) => {
      ...
      try {
        ...
        const hasPermission = checkUserAccess(componentId, userPermissions)
        if (!hasPermission) {
    ...
    async (query: string) => {
      ...
      try {
        ...
        const filtered = allComponents.filter((componentId) => {
          ...
          const category = SPA_CATEGORIES.find((cat) => cat.components.includes(componentId))
  ...
  const executeComponent = useCallback(
    async (componentId: string, params?: any) => {
      try {
  ...
  const getFilteredComponents = useCallback(
    (categoryId: string) => {
      ...
      // Apply search filter
      if (searchQuery.trim()) {
        components = components.filter((componentId) => filteredComponents.includes(componentId))
      }
  ...
  const renderCategory = useCallback(
    (category: (typeof SPA_CATEGORIES)[0]) => {
      // Check if user has permission to access this category
      if (category.adminOnly && !userPermissions.includes("rbac:admin")) {
        return null
      }

      const hasRequiredPermissions = category.permissions.some(
        (permission) => Array.isArray(userPermissions) && userPermissions.includes(permission),
      )
        ...
        <div key={category.id} className="space-y-3">
          <Collapsible open={isActive} onOpenChange={() => setActiveCategory(isActive ? "" : category.id)}>
            <CollapsibleTrigger asChild>
              <Button variant="ghost" className={cn("w-full justify-between p-3 h-auto", isActive && "bg-muted")}>
                <div className="flex items-center gap-3">
    ...
    [userPermissions, activeCategory, getFilteredComponents, renderComponent],
  )
            ...
            >
                    ...
                    {mode !== "mini" && (
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground">
                          {
                            SPA_CATEGORIES.filter(
                              (cat) =>
                                !cat.adminOnly ||
                                (Array.isArray(userPermissions) && userPermissions.includes("rbac:admin")),
                            ).length
                          }{" "}
                          categories
                        </p>
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    <Tooltip>
                      <TooltipTrigger asChild>
                        ...
                        >
                          {isPinned ? <Unpin className="w-4 h-4" /> : <Pin className="w-4 h-4" />}
                        </Button>
                      </TooltipTrigger>
                      <TooltipContent>{isPinned ? "Unpin sidebar" : "Pin sidebar"}</TooltipContent>
                    </Tooltip>
                  ...
                  <div className="flex items-center gap-2 mt-3">
                    <DropdownMenu>
                      <DropdownMenuTrigger asChild>
                        <Button variant="outline" size="sm" className="h-8 flex-1 bg-transparent">
              ...
              <div className="flex-1 overflow-hidden">
                <ScrollArea ref={scrollAreaRef} className="h-full">
                  <div className="p-4 space-y-4">
                          ...
                          {actionHistory.slice(0, 3).map((action) => (
                            ...
                            >
                              ...
                              <div className="flex-1 min-w-0">
                                ...
                                <p className="text-xs text-muted-foreground">{formatActionTime(action.timestamp)}</p>
                              </div>
                            </div>
                          ))}
                        </div>
                    ...
                    {/* SPA Categories */}
                    <div className="space-y-2">{SPA_CATEGORIES.map(renderCategory)}</div>
                  </div>
                </ScrollArea>
              </div>
              ...
              <div className="flex-shrink-0 p-4 border-t border-border">
                <div className="flex items-center justify-between text-xs text-muted-foreground">
                  <span>{loadedComponents.size} components loaded</span>
                  {enableAnalytics && (
                    <span>
                      {Object.values(componentMetrics).reduce((sum, metrics) => sum + (metrics.usageCount || 0), 0)}{" "}
                      actions
                    </span>
                  )}
                </div>
              ...
              <Sheet open={showSettings} onOpenChange={setShowSettings}>
                <SheetContent side={position === "right" ? "left" : "right"} className="w-80">
                  <SheetHeader>
                    ...
                    <SheetDescription>Customize your quick actions sidebar experience</SheetDescription>
                  </SheetHeader>
                      ...
                      <div className="space-y-4">
                        ...
                        <div className="space-y-2">
                          ...
                          <Slider
                            ...
                            className="w-full"
                          />
                          <div className="text-xs text-muted-foreground">{sidebarConfig.width}px</div>
                        </div>
                      </div>
                    </div>
