...
export const RacineRouter: React.FC<RacineRouterProps> = ({
  ...
  useEffect(() => {
    ...
    const trackRouteVisit = async () => {
      try {
        const previousPath = navigationHistory[navigationHistory.length - 1]?.path || '/';
    ...
    trackRouteVisit();
  }, [currentView, currentRoute, enableAnalytics, trackNavigation, currentUser, activeWorkspace, navigationHistory]);
