...
export const GlobalQuickActionsSidebar: React.FC<GlobalQuickActionsSidebarProps> = ({
  ...
  // Stabilize potentially unstable function deps to avoid effect thrash
  const getContextActionsRef = useRef(getContextActions)
  const getActionAnalyticsRef = useRef(getActionAnalytics)
  const getRecentActionsRef = useRef(getRecentActions)
  const getFavoriteActionsRef = useRef(getFavoriteActions)
  const getUsageAnalyticsRef = useRef(getUsageAnalytics)
  const trackEventRef = useRef(trackEvent)

  useEffect(() => { getContextActionsRef.current = getContextActions }, [getContextActions])
  useEffect(() => { getActionAnalyticsRef.current = getActionAnalytics }, [getActionAnalytics])
  useEffect(() => { getRecentActionsRef.current = getRecentActions }, [getRecentActions])
  useEffect(() => { getFavoriteActionsRef.current = getFavoriteActions }, [getFavoriteActions])
  useEffect(() => { getUsageAnalyticsRef.current = getUsageAnalytics }, [getUsageAnalytics])
  useEffect(() => { trackEventRef.current = trackEvent }, [trackEvent])

  // Load initial data and setup (only when visible)
  useEffect(() => {
    if (!isOpen) return
    const initializeSidebar = async () => {
      try {
        ...
        if (enableContextualActions) {
          const contextActions = await getContextActionsRef.current(currentContext)
        ...
        if (enableAnalytics) {
          try {
            const analytics = await getActionAnalyticsRef.current()
        ...
        const [recentActions, favoriteActions] = await Promise.all([
          getRecentActionsRef.current(20),
          getFavoriteActionsRef.current()
        ])
        ...
        if (enableAnalytics) {
          analyticsIntervalRef.current = setInterval(async () => {
            try {
              const metrics = await getUsageAnalyticsRef.current()
  ...
  // Debounced search (only when visible, ignore very short queries)
  useEffect(() => {
    if (!isOpen) return
