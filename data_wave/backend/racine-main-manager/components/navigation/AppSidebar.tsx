...
export const AppSidebar: React.FC<AppSidebarProps> = ({
  ...
  // Defer heavy initialization to idle time to avoid blocking first paint
  const [initialized, setInitialized] = useState(false)
  useEffect(() => {
    let timeoutId: any
    if (typeof (window as any).requestIdleCallback === 'function') {
      ;(window as any).requestIdleCallback(() => setInitialized(true))
    } else {
      // Fallback: defer a bit after first paint
      timeoutId = setTimeout(() => setInitialized(true), 250)
    }
  ...
  // Load user preferences and favorites (after init)
  useEffect(() => {
    ...
    loadUserData()
  }, [initialized, getSidebarPreferences])

  // Load recent navigation history (after init)
  useEffect(() => {
    if (!initialized) return
    ...
    loadRecentItems()
  }, [initialized, getRecentNavigationHistory])
