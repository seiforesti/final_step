...
export function useAIAssistant(p0: string, p1: { context: string; currentBreakpoint: ResponsiveBreakpoint; deviceType: "desktop" | "tablet" | "mobile"; currentLayout: LayoutConfiguration; }, options: UseAIAssistantOptions = {}) {
  ...
  const createAutomationWorkflow = useCallback(async (options: any): Promise<any> => {
    try {
      setLoading('optimization', true);
      setError('optimization', null);

      // Use existing backend service for workflow creation
      const workflow = await aiAssistantAPI.createWorkflow({
        name: options.name,
      ...
      updateState(prev => ({
        ...prev,
        workflowSuggestions: [workflow, ...prev.workflowSuggestions].slice(0, 20)
      }));

      return workflow;
    } catch (error) {
      const message = error instanceof Error ? error.message : 'Failed to create automation workflow';
      setError('optimization', message);
      return null;
    } finally {
      setLoading('optimization', false);
  ...
  const executeAutomationWorkflow = useCallback(async (workflowId: string, options: any): Promise<any> => {
    try {
      setLoading('optimization', true);
      setError('optimization', null);

      // Use existing backend service for workflow execution
      const execution = await aiAssistantAPI.executeWorkflow(workflowId, {
      ...
      const message = error instanceof Error ? error.message : 'Failed to execute automation workflow';
      setError('optimization', message);
      return null;
    } finally {
      setLoading('optimization', false);
  ...
  const updateWorkflowConfiguration = useCallback(async (workflowId: string, updates: any): Promise<any> => {
    try {
      setLoading('optimization', true);
      setError('optimization', null);

      // Use existing backend service for workflow updates
      const workflow = await aiAssistantAPI.updateWorkflow(workflowId, {
        updates: updates,
      ...
      const message = error instanceof Error ? error.message : 'Failed to update workflow configuration';
      setError('optimization', message);
      return null;
    } finally {
      setLoading('optimization', false);
  ...
  const deleteWorkflow = useCallback(async (workflowId: string): Promise<boolean> => {
    try {
      ...
      const result = await aiAssistantAPI.deleteWorkflow(workflowId);
  ...
  const createAutomationRule = useCallback(async (rule: any): Promise<any> => {
    try {
      setLoading('optimization', true);
      setError('optimization', null);

      // Use existing backend service for rule creation
      const newRule = await aiAssistantAPI.createAutomationRule({
        rule: rule
      });
  ...
  const updateAutomationRule = useCallback(async (ruleId: string, updates: any): Promise<any> => {
    try {
      setLoading('optimization', true);
      setError('optimization', null);

      // Use existing backend service for rule updates
      const result = await aiAssistantAPI.updateAutomationRule(ruleId, {
        updates: updates
      });
  ...
  const deleteAutomationRule = useCallback(async (ruleId: string): Promise<boolean> => {
    try {
      ...
      const result = await aiAssistantAPI.deleteAutomationRule(ruleId);
  ...
  const getWorkflowSuggestions = useCallback(async (options: any): Promise<any[]> => {
    try {
      setLoading('recommendations', true);
      setError('recommendations', null);

      // Use existing backend service for workflow suggestions
      const suggestions = await aiAssistantAPI.getWorkflowSuggestions({
        context: options.context,
      ...
      const message = error instanceof Error ? error.message : 'Failed to get workflow suggestions';
      setError('recommendations', message);
      return [];
    } finally {
      setLoading('recommendations', false);
  ...
  const optimizeWorkflow = useCallback(async (workflowId: string, options: any): Promise<any> => {
    try {
      setLoading('optimization', true);
      setError('optimization', null);

      // Use existing backend service for workflow optimization
      const optimization = await aiAssistantAPI.optimizeWorkflow(workflowId, {
      ...
      const message = error instanceof Error ? error.message : 'Failed to optimize workflow';
      setError('optimization', message);
      return null;
    } finally {
      setLoading('optimization', false);
  ...
  const scheduleWorkflow = useCallback(async (workflowId: string, options: any): Promise<any> => {
    try {
      setLoading('optimization', true);
      setError('optimization', null);

      // Use existing backend service for workflow scheduling
      const schedule = await aiAssistantAPI.scheduleWorkflow(workflowId, {
        schedule: options,
      ...
      const message = error instanceof Error ? error.message : 'Failed to schedule workflow';
      setError('optimization', message);
      return null;
    } finally {
      setLoading('optimization', false);
  ...
  const monitorWorkflowExecution = useCallback(async (executionId: string, options?: any): Promise<any> => {
    try {
      // Use existing backend service for execution monitoring
      const execution = await aiAssistantAPI.monitorWorkflowExecution(executionId, {
  ...
  const getWorkflowAnalytics = useCallback(async (timeRange: string = '24h'): Promise<any> => {
    try {
      setLoading('insights', true);
      setError('insights', null);

      // Use existing backend service for workflow analytics
      const analytics = await aiAssistantAPI.getWorkflowAnalytics({
        time_range: timeRange,
      ...
      const message = error instanceof Error ? error.message : 'Failed to get workflow analytics';
      setError('insights', message);
      return null;
    } finally {
      setLoading('insights', false);
