from sqlmodel import SQLModel, Field, Relationship, Column, JSON, String, Text, Index
...
class ComplianceRiskMatrix(SQLModel, table=True):
    """Persisted configuration for the enterprise risk assessment matrix."""
    __tablename__ = "compliance_risk_matrix"

    id: Optional[int] = Field(default=None, primary_key=True)

    # Matrix configuration
    probability_levels: List[Dict[str, Any]] = Field(default_factory=list, sa_column=Column(JSON))
    impact_levels: List[Dict[str, Any]] = Field(default_factory=list, sa_column=Column(JSON))
    risk_matrix: List[List[int]] = Field(default_factory=list, sa_column=Column(JSON))
    categories: List[Dict[str, Any]] = Field(default_factory=list, sa_column=Column(JSON))
    ...
    # Table indexes
    __table_args__ = (
        Index('ix_risk_matrix_updated_at', 'updated_at'),
    )
