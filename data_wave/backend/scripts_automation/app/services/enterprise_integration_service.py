...
try:
    ...
    HAS_NETWORKX = False
    # Enterprise-level fallback graph implementation
    class EnterpriseGraph:
        """Enterprise-level graph implementation with advanced features"""
        
        def __init__(self, directed=True):
            self.directed = directed
            self.nodes = {}  # node_id -> node_data
            self.edges = {}  # (source, target) -> edge_data
            self.incoming_edges = {}  # target -> set of sources
            self.outgoing_edges = {}  # source -> set of targets
        ...
        def add_node(self, node, **attr):
            ...
            self.node_attributes[node] = attr
        ...
        def add_edge(self, source, target, **attr):
            ...
            self.edges[edge_key] = attr
            self.edge_attributes[edge_key] = attr
            
            # Update adjacency lists
            if source not in self.outgoing_edges:
                self.outgoing_edges[source] = set()
            if target not in self.incoming_edges:
                self.incoming_edges[target] = set()
        ...
        def remove_edge(self, source, target):
            ...
            if edge_key in self.edges:
                ...
                del self.edge_attributes[edge_key]
                
                # Update adjacency lists
                if source in self.outgoing_edges and target in self.outgoing_edges[source]:
                    self.outgoing_edges[source].remove(target)
                if target in self.incoming_edges and source in self.incoming_edges[target]:
                    self.incoming_edges[target].remove(source)
        
        def get_node_attributes(self, node):
            """Get attributes of a node"""
            return self.node_attributes.get(node, {})
        
        def get_edge_attributes(self, source, target):
            """Get attributes of an edge"""
            return self.edge_attributes.get((source, target), {})
        
        def neighbors(self, node):
            """Get neighbors of a node"""
            return list(self.outgoing_edges.get(node, set()))
        
        def predecessors(self, node):
            """Get predecessors of a node"""
            return list(self.incoming_edges.get(node, set()))
        ...
        def degree(self, node):
            ...
            in_degree = len(self.incoming_edges.get(node, set()))
            out_degree = len(self.outgoing_edges.get(node, set()))
            return in_degree + out_degree
        
        def in_degree(self, node):
            """Get in-degree of a node"""
            return len(self.incoming_edges.get(node, set()))
        
        def out_degree(self, node):
            """Get out-degree of a node"""
            return len(self.outgoing_edges.get(node, set()))
        ...
        def copy(self):
            ...
            new_graph = EnterpriseGraph(directed=self.directed)
            new_graph.nodes = self.nodes.copy()
            new_graph.edges = self.edges.copy()
            new_graph.incoming_edges = {k: v.copy() for k, v in self.incoming_edges.items()}
            new_graph.outgoing_edges = {k: v.copy() for k, v in self.outgoing_edges.items()}
            new_graph.node_attributes = self.node_attributes.copy()
            new_graph.edge_attributes = self.edge_attributes.copy()
            return new_graph
        
        def subgraph(self, nodes):
            """Create a subgraph with specified nodes"""
            subgraph = EnterpriseGraph(directed=self.directed)
            for node in nodes:
                if self.has_node(node):
                    subgraph.add_node(node, **self.get_node_attributes(node))
            
            for edge in self.edges:
                if edge[0] in nodes and edge[1] in nodes:
                    subgraph.add_edge(*edge, **self.get_edge_attributes(*edge))
        ...
        def to_undirected(self):
            ...
            undirected = EnterpriseGraph(directed=False)
            for node, attrs in self.nodes.items():
                undirected.add_node(node, **attrs)
            
            for edge, attrs in self.edges.items():
                undirected.add_edge(*edge, **attrs)
                # Add reverse edge for undirected graph
                undirected.add_edge(edge[1], edge[0], **attrs)
            
            return undirected
    
    # Create NetworkX-compatible interface
    class NetworkXInterface:
        @staticmethod
        def DiGraph():
            return EnterpriseGraph(directed=True)
        
        @staticmethod
        def Graph():
            return EnterpriseGraph(directed=False)
    
    nx = NetworkXInterface()
...
try:
    ...
    class DataSourceConnectionService:
        async def test_connection_async(self, data_source_id): 
            ...
            try:
                # Test database connectivity with enterprise-level operations
                if hasattr(self, 'db_session') and self.db_session:
                    try:
                        # Comprehensive database health check
                        health_checks = []
                        
                        # 1. Connectivity and permissions test
                        test_query = "SELECT 1"
                        result = self.db_session.execute(text(test_query))
                        health_checks.append({"test": "connectivity", "status": "passed" if result else "failed"})
                        
                        # 2. Performance test
                        start_time = time.time()
                        perf_query = "SELECT COUNT(*) FROM information_schema.tables"
                        perf_result = self.db_session.execute(text(perf_query))
                        perf_time = time.time() - start_time
                        health_checks.append({
                            "test": "performance", 
                            "status": "passed" if perf_time < 1.0 else "slow",
                            "response_time_ms": round(perf_time * 1000, 2)
                        })
                        
                        # 3. Schema access test
                        schema_query = "SELECT schema_name FROM information_schema.schemata LIMIT 5"
                        schema_result = self.db_session.execute(text(schema_query))
                        health_checks.append({
                            "test": "schema_access", 
                            "status": "passed" if schema_result else "failed"
                        })
                        ...
                        try:
                            ...
                            health_checks.append({"test": "transaction_handling", "status": "passed"})
                        except Exception as tx_error:
                            health_checks.append({"test": "transaction_handling", "status": "failed", "error": str(tx_error)})
                        
                        # 5. Connection pool test
                        pool_query = "SELECT version()"
                        pool_result = self.db_session.execute(text(pool_query))
                        health_checks.append({
                            "test": "connection_pool", 
                            "status": "passed" if pool_result else "failed"
                        })
                        ...
                        db_status = "failed"
                        db_details = {"error": str(db_error)}
                else:
                    db_status = "no_session"
                    db_details = {"error": "Database session not available"}
                
                # Test external service connectivity with enterprise-level health checks
                    ...
                    for service_name, service_config in self.external_services.items():
                        try:
                            ...
                            if service_config.get('test_endpoint'):
                                ...
                                service_health["response_times"]["connectivity"] = round(response_time * 1000, 2)
                                
                                if response.status_code == 200:
                                    service_health["health_checks"].append({
                                        "test": "connectivity",
                                        "status": "passed",
                            ...
                            # 2. Authentication test (if credentials provided)
                            if service_config.get('auth_endpoint') and service_config.get('credentials'):
                                try:
                                    auth_start = time.time()
                                    auth_response = requests.post(
                                        service_config['auth_endpoint'],
                                        json=service_config['credentials'],
                                        timeout=10
                                    )
                                    auth_time = time.time() - auth_start
                                    
                                    service_health["response_times"]["authentication"] = round(auth_time * 1000, 2)
                            ...
                            # 3. API functionality test (if available)
                            if service_config.get('api_test_endpoint'):
                                try:
                                    api_start = time.time()
                                    api_response = requests.get(service_config['api_test_endpoint'], timeout=15)
                                    api_time = time.time() - api_start
                                    
                                    service_health["response_times"]["api_functionality"] = round(api_time * 1000, 2)
                            ...
                            total_checks = len(service_health["health_checks"])
                            passed_checks = sum(1 for check in service_health["health_checks"] if check["status"] == "passed")
                            ...
                            service_health["health_score"] = round((passed_checks / total_checks) * 100, 2) if total_checks > 0 else 0
                            
                            external_details[service_name] = service_health
                            
                            # Set overall external status
                            if service_health["status"] == "healthy":
                                external_status = f"{service_name}:healthy"
                            elif service_health["status"] == "degraded":
                                external_status = f"{service_name}:degraded"
                            else:
                                external_status = f"{service_name}:unhealthy"
                                
                        except Exception as e:
                            external_details[service_name] = {
                                "service_name": service_name,
                                "status": "failed",
                                "error": str(e),
                                "health_checks": [],
                ...
                return {
                    "success": True, 
                    "message": "Enterprise-level connection test completed",
                    "database_status": db_status,
                    "database_details": db_details,
                    "external_services_status": external_status,
                    "external_services_details": external_details,
                    "overall_health_score": self._calculate_overall_health_score(db_details, external_details),
    ...
    def _calculate_overall_health_score(self, db_details, external_details):
        """Calculate overall system health score from database and external service details"""
        try:
            scores = []
            weights = []
            
            # Database health score
            if isinstance(db_details, dict) and "health_score" in db_details:
                scores.append(db_details["health_score"])
                weights.append(0.6)  # Database has higher weight
            
            # External services health scores
            if isinstance(external_details, dict):
                for service_name, service_details in external_details.items():
                    if isinstance(service_details, dict) and "health_score" in service_details:
                        scores.append(service_details["health_score"])
                        weights.append(0.4 / len(external_details))  # Distribute weight among services
...
try:
    ...
    class ComplianceRuleService:
        async def get_applicable_rules_for_data_source(self, ds_id, metadata):
            # Enterprise-grade derivation of applicable controls using metadata signals
            frameworks = (metadata or {}).get("frameworks", ["SOC2"])
            ds_tags = set((metadata or {}).get("tags", []))
            applicable = []
            for framework in frameworks:
                # Map common frameworks to core controls
                if framework.upper() in ("GDPR", "CCPA"):
                    applicable.extend([
                        {"framework": framework, "rule": "data_minimization"},
                        {"framework": framework, "rule": "subject_access_request"},
                        {"framework": framework, "rule": "right_to_erasure"},
                    ])
                elif framework.upper() in ("SOC2", "ISO27001"):
                    applicable.extend([
                        {"framework": framework, "rule": "access_control"},
                        {"framework": framework, "rule": "audit_logging"},
                        {"framework": framework, "rule": "encryption_at_rest"},
                    ])
            # Add controls based on sensitivity tags
            if any(t.lower() in ("pii", "phi", "confidential") for t in ds_tags):
                applicable.append({"framework": "internal", "rule": "data_masking"})
                applicable.append({"framework": "internal", "rule": "tokenization"})
            # De-duplicate
            seen = set()
            deduped = []
            for item in applicable:
                key = (item["framework"], item["rule"]) 
                if key not in seen:
                    seen.add(key)
                    deduped.append(item)
            return deduped
        async def analyze_data_source_compliance(self, ds_id, metadata, rules):
            # Enterprise scoring using signals from encryption, access, retention, lineage
            signals = {
                "has_encryption": bool((metadata or {}).get("encryption", {}).get("at_rest")),
                "has_access_controls": bool((metadata or {}).get("access", {}).get("rbac")),
                "has_retention_policy": bool((metadata or {}).get("retention", {}).get("policy_days")),
                "has_lineage": bool((metadata or {}).get("lineage", {}).get("upstream")),
                "sensitive": any(tag.lower() in ("pii", "phi", "confidential") for tag in (metadata or {}).get("tags", [])),
            }
            base = 50
            base += 10 if signals["has_encryption"] else -5
            base += 10 if signals["has_access_controls"] else -5
            base += 5 if signals["has_retention_policy"] else -5
            base += 5 if signals["has_lineage"] else 0
            # Penalize missing controls when sensitive
            if signals["sensitive"]:
                if not signals["has_encryption"]:
                    base -= 15
                if not signals["has_access_controls"]:
                    base -= 15
            # Rule coverage contribution
            coverage = len(rules or [])
            base += min(20, coverage * 3)
            score = float(max(0, min(100, base)))
            return {
                "compliance_score": score,
                "controls": [r.get("rule") for r in (rules or [])],
                "signals": signals,
            }
        async def generate_compliance_recommendations(self, ds_id, analysis):
            recs = []
            score = (analysis or {}).get("compliance_score", 0)
            signals = (analysis or {}).get("signals", {})
            if score < 85:
                recs.append("Enable automated evidence collection")
            if not signals.get("has_encryption"):
                recs.append("Enforce encryption at rest and in transit with KMS integration")
            if not signals.get("has_access_controls"):
                recs.append("Implement RBAC/ABAC and privileged access management")
            if not signals.get("has_retention_policy"):
                recs.append("Define retention policy and automated purging workflows")
            return recs
...
try:
    ...
    class ClassificationService:
        async def classify_data_source(self, ds_id, metadata, **kwargs):
            # Enterprise heuristics with regex and context patterns
            import re
            pii_patterns = [
                ("email", re.compile(r"email|e[-_\s]?mail", re.I)),
                ("phone", re.compile(r"phone|mobile|msisdn|contact[_-]?number", re.I)),
                ("ssn", re.compile(r"ssn|social[_-]?security", re.I)),
                ("credit_card", re.compile(r"card|ccnum|pan|payment", re.I)),
                ("name", re.compile(r"first[_-]?name|last[_-]?name|full[_-]?name", re.I)),
                ("address", re.compile(r"address|street|city|postal|zip", re.I)),
            ]
            cols = []
                ...
                for c in t.get("columns", []):
                    ...
                    for tag, pat in pii_patterns:
                        if pat.search(col_name):
                            label = "pii"
                            break
                    # Elevate sensitivity if table itself is sensitive
                    if re.search(r"user|customer|patient|employee", table_name or "", re.I):
                        if label == "public":
                            label = "confidential"
                    cols.append({"table": table_name, "column": col_name, "sensitivity": label})
        ...
        async def apply_auto_classification_rules(self, ds_id, results):
            ...
            for item in results.get("column_classifications", []):
                if item.get("sensitivity") in ("pii", "confidential"):
                    actions.add("masking_policy")
                    actions.add("access_control_policy")
...
try:
    ...
    class EnterpriseScanRuleService:
        async def generate_rules_for_data_source(self, ds_id, metadata):
            # Generate rules using schema signals and sensitivity mapping
            rules = []
            ...
            for table in tables:
                ...
                checks = ["not_null"]
                # Primary key/unique
                if any(c.get("is_primary_key") or c.get("unique") for c in table.get("columns", [])):
                    checks.append("unique_key")
                # Foreign key
                if any(c.get("foreign_key") for c in table.get("columns", [])):
                    checks.append("foreign_key_consistency")
                # Sensitive data enforcement
                if any(re for re in [
                    any(any(k in (col.get("name", "").lower()) for k in ["email", "ssn", "phone"]) for col in table.get("columns", []))
                ]):
                    checks.append("masking_policy")
                    checks.append("row_level_security")
                rules.append({
                    "table": tname,
                    "checks": sorted(set(checks)),
                    "priority": "high" if "masking_policy" in checks else "medium",
                })
            return rules
...
class EnterpriseIntegrationService:
    ...
    async def _calculate_performance_statistics(self):
        """Calculate performance statistics with real enterprise metrics"""
        try:
            ...
            successful_requests = sum(m.get('successful_requests', 0) for m in service_metrics.values())
            ...
            self.metrics.success_rate = (successful_requests / total_requests * 100) if total_requests > 0 else 100.0
            
        except Exception as e:
            logger.warning(f"Error calculating performance statistics: {e}")
        
    def _calculate_performance_trend(self):
        """Calculate performance trend with real enterprise analysis"""
        try:
            ...
            historical_data = self.metrics.get_historical_data(days=30)
            
            if not historical_data:
                return "stable"
            
            # Calculate trend using advanced analytics
            trend_analysis = analytics_service.calculate_performance_trend(historical_data)
