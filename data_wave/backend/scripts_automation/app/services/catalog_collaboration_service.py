...
class CatalogCollaborationService:
    ...
    def _auto_assign_reviewers(session: Session, review_id: int, review_type: ReviewType):
        ...
        try:
            ...
            for r in reviewers:
                ...
                workload = len(active_assignments)
                # Prefer leaders/admins slightly if available
                role_weight = 0.9 if getattr(r, "role", "member") in ("leader", "admin") else 1.0
                # Effective score: lower is better
                score = workload * role_weight
                reviewer_scores.append((r.id, score))

            reviewer_scores.sort(key=lambda x: x[1])
            best_reviewer_id = reviewer_scores[0][0]
    ...
    def _find_suitable_expert(session: Session, network_id: int, topic: str) -> Optional[DomainExpert]:
        ...
        try:
            ...
            # Prepare topic embedding once
            def safe_text_join(values: List[str]) -> str:
            ...
            async def embed(text: str) -> List[float]:
                ...
                result = await svc.generate_embeddings(text=text, embedding_dimensions=128, include_metadata=False)
                return result.get("embedding_vector", [])

            import asyncio as _asyncio
            topic_vector = _asyncio.get_event_loop().run_until_complete(embed(topic)) if EmbeddingService else []
            ...
            candidates: List[Tuple[DomainExpert, float]] = []
            for ex in experts:
                profile_text = safe_text_join([
                    getattr(ex, "bio", ""),
                    " ".join(getattr(ex, "expertise_areas", []) or []),
                    " ".join(getattr(ex, "tags", []) or []),
                    getattr(ex, "title", ""),
                ])
                ex_vec = _asyncio.get_event_loop().run_until_complete(embed(profile_text)) if EmbeddingService else []
                sim = cosine(topic_vector, ex_vec) if topic_vector and ex_vec else 0.0
                    ...
                    open_reqs = session.execute(
                        select(ConsultationRequest).where(
                            and_(
                                ...
                                ConsultationRequest.status.in_(["matched", "requested"])  # type: ignore
                            )
                        )
                    ).all()
                    availability = 1.0 / (1.0 + len(open_reqs))
                except Exception:
                    availability = 0.8
                ...
                ex_domains = set([d.lower() for d in getattr(ex, "domains", []) or []])
                domain_overlap = 1.0 if any(tok in " ".join(ex_domains) for tok in domain_tokens) else 0.0

                score = 0.6 * sim + 0.25 * availability + 0.15 * domain_overlap
