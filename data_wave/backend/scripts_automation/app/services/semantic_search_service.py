...
class SemanticSearchService:
    ...
    # ===== Compatibility helpers for callers expecting direct scoring =====
    async def calculate_semantic_similarity(self, query: str, item_name: str, item_description: str) -> float:
        """
        Compute cosine similarity between the query embedding and the item's textual embedding.
        Returns a score in 0..1.
        """
        try:
            text = " ".join([t for t in [item_name, item_description] if t])
            if not text.strip() or not query.strip():
                return 0.0
            query_emb = await self._get_query_embedding(query)
            item_emb = await self._get_query_embedding(text)
            # cosine similarity
            num = float((query_emb * item_emb).sum())
            den = float((np.linalg.norm(query_emb) * np.linalg.norm(item_emb)) or 0.0)
            if den == 0.0:
                return 0.0
            sim = max(0.0, min(1.0, num / den))
            return float(sim)
        except Exception as exc:
            logger.warning(f"calculate_semantic_similarity fallback (0): {exc}")
            return 0.0

    async def calculate_tag_relevance(self, query: str, tags: List[str]) -> float:
        """
        Simple relevance score based on overlap of query terms with provided tags. Returns 0..1.
        """
        try:
            if not tags or not query:
                return 0.0
            q = set([w for w in re.split(r"\W+", query.lower()) if len(w) > 2])
            t = set([str(tag).lower() for tag in tags])
            if not q or not t:
                return 0.0
            overlap = len(q & t)
            return float(min(1.0, overlap / max(1, len(t))))
        except Exception:
            return 0.0
    ...
    # Collaboration integration: document indexing used by collaboration service
    async def index_document(self, document_id: str, content: Dict[str, Any], metadata: Dict[str, Any]) -> Dict[str, Any]:
        try:
            # Build simple text for embedding
            title = str(content.get("title", ""))
            text = str(content.get("content", ""))
            combined = f"{title}\n{text}"

            # Generate and store
            embedding = await self._get_query_embedding(combined)
            # Store in local maps for search methods already implemented
            if not hasattr(self, 'asset_texts'):
                self.asset_texts = {}
            self.asset_texts[document_id] = combined

            # Add to FAISS
            if document_id not in getattr(self, 'asset_id_to_index', {}):
                index_position = self.semantic_index.ntotal
                self.semantic_index.add(embedding.reshape(1, -1).astype('float32'))
                self.index_to_asset_id[index_position] = document_id
                self.asset_id_to_index[document_id] = index_position
                self.asset_embeddings[document_id] = embedding

            # Store metadata for entity-based search
            self.asset_metadata[document_id] = {
                "asset_id": document_id,
                "name": title,
                "description": text[:500],
                "tags": metadata.get("tags", []),
                "schema": {},
                "last_modified": content.get("updated_at") or content.get("created_at") or datetime.utcnow().isoformat(),
            }

            return {"success": True, "document_id": document_id}
        except Exception as exc:
            logger.error(f"index_document failed: {exc}")
