...
class RacineCollaborationService:
    ...
    async def _broadcast_session_event(
        ...
        # Broadcast via enterprise WebSocket/Notification services when available
        try:
            from app.services.websocket_service import WebSocketService
            ws = WebSocketService()
            payload = {
                'type': f'collab:{event_type}',
                'session_id': session_id,
                'data': event_data,
                'timestamp': datetime.utcnow().isoformat()
            }
            await ws.broadcast_json(payload, exclude_user=exclude_user)
        except Exception:
            pass
        # Always track event for analytics
