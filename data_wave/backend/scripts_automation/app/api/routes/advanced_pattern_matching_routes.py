...
# Service dependencies - deferred to prevent startup failures
pattern_matching_service = None
intelligent_pattern_service = None
rule_optimization_service = None
scan_intelligence_service = None
metrics_collector = MetricsCollector()
logger = logging.getLogger(__name__)

def _ensure_services():
    """Ensure services are initialized when needed"""
    global pattern_matching_service, intelligent_pattern_service, rule_optimization_service, scan_intelligence_service
    
    if pattern_matching_service is None:
        pattern_matching_service = AdvancedPatternMatchingService()
    if intelligent_pattern_service is None:
        intelligent_pattern_service = IntelligentPatternService()
    if rule_optimization_service is None:
        rule_optimization_service = RuleOptimizationService()
    if scan_intelligence_service is None:
