...
async def stream_rule_metrics(
    ...
    try:
        ...
        async def generate_metrics():
            """Generate streaming metrics data"""
            while True:
                try:
                    # In a real implementation, fetch actual metrics from database
                    metrics_data = {
                        "timestamp": datetime.utcnow().isoformat(),
                        "rule_metrics": [],
                        "system_metrics": {
                    ...
                    if rule_ids:
                        for rule_id in rule_ids:
                            metrics_data["rule_metrics"].append({
                                "rule_id": rule_id,
                                "executions_count": 0,
...
async def get_rule_analytics(
    ...
    try:
        ...
        analytics_result = {
            ...
            "summary_statistics": {
                "total_rules_analyzed": len(request.rule_ids) if request.rule_ids else 0,
