...
from app.db_session import get_db  # Use FastAPI dependency for session
import requests
...
# Import user_has_permission for non-RBAC endpoints
from app.services.role_service import user_has_permission

router = APIRouter(prefix="/sensitivity-labels", tags=["Sensitivity Labels"])

# Register the RBAC router (all /rbac endpoints and user/role/permission management)
...
def create_label(label: schemas.SensitivityLabelCreate, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    # Only users with 'create' permission on 'label' and matching row-level conditions can create
    conditions = {"scope": getattr(label, "scope", None), "color": getattr(label, "color", None)}
    if not user_has_permission(db, current_user, "create", "label", conditions=conditions):
        raise HTTPException(status_code=403, detail="Not authorized to create label with these attributes")
...
def list_labels(
    ...
    # Only users with 'view' permission on 'label' and matching row-level conditions can list
    scopes_list = scopes.split(",") if scopes else []
    colors_list = colors.split(",") if colors else []
    date_from_dt = datetime.fromisoformat(date_from) if date_from else None
    date_to_dt = datetime.fromisoformat(date_to) if date_to else None
    # Example: check permission for each scope/color (row-level)
    for scope in scopes_list or [None]:
        for color in colors_list or [None]:
            conditions = {"scope": scope, "color": color}
            if not user_has_permission(db, current_user, "view", "label", conditions=conditions):
                raise HTTPException(status_code=403, detail=f"Not authorized to view labels with scope={scope} and color={color}")
...
def create_proposal(proposal: schemas.LabelProposalCreate, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    # Only users with 'create' permission on 'proposal' and matching row-level conditions can create
    conditions = {"object_type": getattr(proposal, "object_type", None)}
    if not user_has_permission(db, current_user, "create", "proposal", conditions=conditions):
        raise HTTPException(status_code=403, detail="Not authorized to create proposal for this object type")
    return crud.create_proposal(db, proposal)

@router.get("/proposals/", response_model=List[schemas.LabelProposal])
def list_proposals(object_type: Optional[str] = None, object_id: Optional[str] = None, status: Optional[models.LabelStatus] = None, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    # Only users with 'view' permission on 'proposal' and matching row-level conditions can view
    conditions = {"object_type": object_type}
    if not user_has_permission(db, current_user, "view", "proposal", conditions=conditions):
        raise HTTPException(status_code=403, detail="Not authorized to view proposals for this object type")
...
def update_proposal_status(proposal_id: int, status: models.LabelStatus, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    # Only users with 'edit' permission on 'proposal' can update status
    proposal = crud.get_proposal_by_id(db, proposal_id)
    if not proposal:
        raise HTTPException(status_code=404, detail="Proposal not found")
    conditions = {"object_type": getattr(proposal, "object_type", None)}
    if not user_has_permission(db, current_user, "edit", "proposal", conditions=conditions):
        raise HTTPException(status_code=403, detail="Not authorized to update proposal status")
    return crud.update_proposal_status(db, proposal_id, status)

@router.post("/proposals/bulk", response_model=List[schemas.LabelProposal])
def create_bulk_proposals(proposals: List[schemas.LabelProposalCreate], db: Session = Depends(get_db)):
    """
...
def create_audit(audit: schemas.LabelAuditCreate, db: Session = Depends(get_db)):
...
def create_review(review: schemas.LabelReviewCreate, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    # Only users with 'create' permission on 'review' and matching row-level conditions can create
    conditions = {"proposal_id": getattr(review, "proposal_id", None)}
    if not user_has_permission(db, current_user, "create", "review", conditions=conditions):
        raise HTTPException(status_code=403, detail="Not authorized to create review for this proposal")
    return crud.create_review(db, review)

@router.get("/reviews/{proposal_id}", response_model=List[schemas.LabelReview])
def get_reviews(proposal_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    # Only users with 'view' permission on 'review' and matching row-level conditions can view
    conditions = {"proposal_id": proposal_id}
    if not user_has_permission(db, current_user, "view", "review", conditions=conditions):
        raise HTTPException(status_code=403, detail="Not authorized to view reviews for this proposal")
    return crud.get_reviews(db, proposal_id)

@router.patch("/reviews/{review_id}/complete", response_model=schemas.LabelReview)
def complete_review(review_id: int, note: Optional[str] = Body(None), db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
...
def escalate_review(review_id: int, note: Optional[str] = Body(None), db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    review = crud.get_review_by_id(db, review_id)
    if not review:
        raise HTTPException(status_code=404, detail="Review not found")
...
def bulk_review(action: str = Body(...), review_ids: List[int] = Body(...), note: Optional[str] = Body(None), db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
...
def assign_review(review_id: int, assignee: str = Body(...), db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    review = crud.get_review_by_id(db, review_id)
    if not review:
        raise HTTPException(status_code=404, detail="Review not found")
...
def comment_review(review_id: int, comment: str = Body(...), db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    review = crud.get_review_by_id(db, review_id)
    if not review:
        raise HTTPException(status_code=404, detail="Review not found")
...
def get_proposal_status_api(proposal_id: int, db: Session = Depends(get_db)):
    try:
...
# --- ML Model & Feedback (additions from api_old2.txt, only if missing) ---
@router.post("/ml-train")
def ml_train(db: Session = Depends(get_db)):
    """
    Train the ML model using historical proposals and labels.
    """
    proposals = db.query(models.LabelProposal).filter(models.LabelProposal.status == models.LabelStatus.APPROVED).all()
    X = []
    y = []
    for p in proposals:
        metadata = {
        ...
        X.append(extract_features_from_metadata(metadata))
        y.append(p.label_id)
    if X and y:
        import numpy as np
        X_np = np.array(X)
        version = ml_suggestion_service.train(X_np, y, db)
        return {"status": "trained", "samples": len(y), "version": version}
    return {"status": "no data"}

@router.post("/ml-retrain")
def ml_retrain_from_feedback(db: Session = Depends(get_db)):
    """
    Retrain the ML model using all feedback in the database.
    """
    version = ml_suggestion_service.retrain_from_feedback(db)
    if version:
        return {"status": "retrained", "version": version}
    return {"status": "no feedback data"}

@router.get("/ml-feedback", response_model=List[dict])
def list_ml_feedback(db: Session = Depends(get_db)):
    """
    List all ML feedback for admin/model monitoring.
    """
...
def get_ml_model_status(db: Session = Depends(get_db)):
    """
    Get current ML model status and metrics.
    """
    model = db.query(models.MLModelVersion).order_by(models.MLModelVersion.trained_at.desc()).first()
    if not model:
...
@router.get("/ml-feedback/audit")
def audit_ml_feedback(
    skip: int = 0,
    limit: int = 100,
    user_email: str = None,
    label: str = None,
    ...
    """
    List/audit all ML feedback and retrain actions.
    """
    q = db.query(models.Feedback)
    if user_email:
        q = q.filter(models.Feedback.user_email == user_email)
    if label:
        q = q.filter(models.Feedback.actual_label == label)
    feedbacks = q.order_by(models.Feedback.created_at.desc()).offset(skip).limit(limit).all()
    return feedbacks
...
# --- Lineage & Impact Analysis (additions from api_old2.txt, only if missing) ---
@router.get("/lineage/{object_type}/{object_id}")
def get_lineage(object_type: str, object_id: str, direction: str = Query("both", enum=["upstream", "downstream", "both"]), db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    """
...
def get_impact(object_type: str, object_id: str, recursive: bool = Query(False), db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    """
...
# --- Advanced Search & Bulk Import (additions from api_old2.txt, only if missing) ---
...
# --- WebSocket (additions from api_old2.txt, only if missing) ---
...
# --- Catalog Tree Router Registration (additions from api_old2.txt, only if missing) ---
...
def list_ml_model_versions(db: Session = Depends(get_db)):
    """
...
def set_active_ml_model_version(version: str = Body(...), db: Session = Depends(get_db)):
    """
...
# --- Lineage & Impact Analytics Endpoints ---

@router.get("/lineage/{object_type}/{object_id}")
def get_lineage_graph(object_type: str, object_id: str, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    """
    Get the lineage graph for a given object (table/column).
    """
    try:
        from .lineage import get_lineage_graph as lineage_graph
        return lineage_graph(db, object_type, object_id)
    except ImportError:
        raise HTTPException(status_code=500, detail="Lineage module not available")

@router.get("/impact/{object_type}/{object_id}")
def get_impact_analysis(object_type: str, object_id: str, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    """
    Get impact analysis for a given object (table/column).
    """
    try:
        from .lineage import get_impact_analysis as impact_analysis
        return impact_analysis(db, object_type, object_id)
    except ImportError:
        raise HTTPException(status_code=500, detail="Lineage module not available")
...
def label_coverage_analytics(db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    """
    Get analytics on label coverage (e.g., % of objects labeled).
    """
    try:
        from .analytics import get_label_coverage
        return get_label_coverage(db)
    except ImportError:
        raise HTTPException(status_code=500, detail="Analytics module not available")

@router.get("/analytics/pending-reviews")
def pending_reviews_analytics(db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    """
    Get analytics on pending reviews (e.g., count, by user/type).
    """
    try:
        from .analytics import get_pending_reviews
        return get_pending_reviews(db)
    except ImportError:
        raise HTTPException(status_code=500, detail="Analytics module not available")

@router.get("/analytics/history")
def label_change_history_analytics(db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    """
    Get analytics on label change history (e.g., timeline, by user/type).
    """
    try:
        from .analytics import get_label_change_history
        return get_label_change_history(db)
    except ImportError:
        raise HTTPException(status_code=500, detail="Analytics module not available")

# --- End of restored endpoints ---
