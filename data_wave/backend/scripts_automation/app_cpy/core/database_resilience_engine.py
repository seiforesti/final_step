...
class IntelligentConnectionPool:
    ...
    def _get_optimized_connect_args(self) -> Dict:
        ...
        # PgBouncer rejects startup parameter 'options'; only set when not using PgBouncer
        try:
            if not self._detect_pgbouncer():
                args["options"] = "-c statement_timeout=30000 -c idle_in_transaction_session_timeout=30000"
        except Exception:
            # Be safe and skip options if detection fails
            pass
        return args
    ...
    def _collect_metrics(self):
        ...
        try:
            ...
            if hasattr(engine, 'pool') and engine.pool:
                pool = engine.pool
                # Safely handle NullPool which doesn't expose size()/overflow()
                try:
                    size_val = pool.size()
                    checked_out_val = pool.checkedout()
                    checked_in_val = pool.checkedin()
                    overflow_val = pool.overflow()
                    total_cap = max(1, size_val + overflow_val)
                    util_pct = (checked_out_val / total_cap) * 100
                except Exception:
                    # Likely NullPool or non-standard pool; treat as PgBouncer-managed
                    size_val = 0
                    checked_out_val = 0
                    checked_in_val = 0
                    overflow_val = 0
...
class QueryOptimizer:
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        # When there have been no cache lookups yet, report 100% to avoid noisy low-cache alerts at idle
        cache_hit_rate = (self.cache_hits / total_cache_lookups) * 100 if total_cache_lookups > 0 else 100.0
...
class DatabaseResilienceEngine:
    ...
    @contextmanager
    def get_sync_session(self, priority: QueryPriority = QueryPriority.NORMAL):
        """Get a synchronous database session with resilience features"""
        endpoint = f"sync_session_{priority.name.lower()}"
        session = None
        start_time = time.time()
        ...
        try:
            ...
            try:
                ...
                # Ensure session is not left in bad state
                session.rollback()
                raise
        except Exception as e:
            self.total_errors += 1
    ...
    def force_cleanup(self):
        """Force cleanup of connections and reset leak detector state"""
        try:
            for engine in list(self.connection_pool.engines):
                try:
                    engine.dispose()
                except Exception:
                    pass
            # Recreate primary engine to ensure fresh connections
            self.connection_pool.engines = []
            self.connection_pool._create_primary_engine()
            # Reset leak detector tracking
            self.connection_pool.connection_leak_detector.active_connections.clear()
        except Exception as e:
