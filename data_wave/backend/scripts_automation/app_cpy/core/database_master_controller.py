...
# NOTE: Avoid importing app.db_session at module import time to prevent circular imports.
...
class DatabaseMasterController:
    ...
    def get_session(self, priority: QueryPriority = QueryPriority.NORMAL, timeout: float = 30.0):
        ...
        try:
            ...
            # Get session directly from resilience engine to avoid circular imports
            if self.resilience_engine:
                with self.resilience_engine.get_sync_session(priority=priority) as session:
                    self.operation_stats['total_operations'] += 1
                    yield session
                    self.operation_stats['successful_operations'] += 1
            else:
                # Fallback to direct SQLAlchemy session if resilience engine not available
            ...
            logger.error(f"❌ Session operation failed: {type(e).__name__}: {e}", exc_info=True)
    ...
    async def get_async_session(self, priority: QueryPriority = QueryPriority.NORMAL):
        ...
        try:
            # Get session directly from resilience engine to avoid circular imports
            if self.resilience_engine:
                async with self.resilience_engine.get_async_session(priority=priority) as session:
                    yield session
            else:
                # Fallback to direct SQLAlchemy async session if resilience engine not available
                from sqlalchemy.ext.asyncio import create_async_engine, AsyncSession
                from sqlalchemy.orm import sessionmaker
                async_engine = create_async_engine(self.database_url.replace('postgresql+psycopg2://', 'postgresql+asyncpg://'))
    ...
    def execute_query(
        ...
        try:
            if use_scheduler and self.query_scheduler:
                ...
                # Direct execution through resilience engine or fallback
                from sqlalchemy import text
                if self.resilience_engine:
                    with self.resilience_engine.get_session(priority=priority) as session:
                        exec_result = session.execute(text(query), params or {})
                        result = exec_result.fetchall() if exec_result.returns_rows else exec_result.rowcount
                else:
            ...
            self.operation_stats['failed_operations'] += 1
            logger.error(f"❌ Query execution failed: {type(e).__name__}: {e}", exc_info=True)
            raise
    ...
    def _should_trigger_emergency_protocols(self, error: Exception) -> bool:
        """Decide if emergency protocols should be triggered for the given error."""
        try:
            err_str = str(error).lower()
            triggers = (
                "circuit breaker is open" in err_str or
                "too many connections" in err_str or
                "connection refused" in err_str or
                "timeout" in err_str or
                "database_unavailable" in err_str
            )
            return triggers
        except Exception:
            return False

    def _trigger_emergency_protocols(self, error: Exception) -> None:
        """Apply immediate mitigations to keep service responsive."""
        try:
            logger.critical("🚨 ACTIVATING EMERGENCY MODE!")
            self.current_mode = OperationMode.EMERGENCY
            # Reduce query concurrency if scheduler exists
            if hasattr(self, 'query_scheduler') and self.query_scheduler:
                self.query_scheduler.max_concurrent_queries = max(2, int(self.query_scheduler.max_concurrent_queries * 0.5))
            # Force database cleanup to drop stale connections
            try:
                if self.resilience_engine:
                    self.resilience_engine.force_cleanup()
            except Exception:
                pass
        except Exception as e:
            logger.error(f"Emergency protocol application failed: {e}")
