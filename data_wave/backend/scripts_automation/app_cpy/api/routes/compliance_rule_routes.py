...
async def get_insights(
    ...
    try:
        from app.services.compliance_production_services import ComplianceAnalyticsService
        insights = []
        # Reuse trend/statistics to synthesize actionable insights
        trends = ComplianceAnalyticsService.get_compliance_trends(session, rule_id=rule_id, days=days or 30)
        stats = ComplianceAnalyticsService.get_dashboard_statistics(session)
        if trends:
            last = trends[-1]
            if last.get("compliance_score", 1.0) < 0.8:
                insights.append({
                    "id": f"insight_low_score_{last['date']}",
                    "type": "compliance",
                    "title": "Low Compliance Score Detected",
                    "description": f"Score {last['compliance_score']} on {last['date']}",
                    "severity": "high" if last['compliance_score'] < 0.6 else "medium",
                    "created_at": datetime.now().isoformat(),
                })
            if last.get("issues_found", 0) > 0:
                insights.append({
                    "id": f"insight_issues_{last['date']}",
                    "type": "issues",
                    "title": "Open Issues Found",
                    "description": f"{last['issues_found']} issues detected on {last['date']}",
                    "severity": "medium",
                    "created_at": datetime.now().isoformat(),
                })
        ...
        if summary.get("critical_issues", 0) > 0:
            insights.append({
                "id": "insight_critical_issues",
                "type": "risk",
                "title": "Critical Issues Present",
                "description": f"{summary.get('critical_issues')} critical issues remain open",
                "severity": "critical",
                "created_at": datetime.now().isoformat(),
            })
        return insights
