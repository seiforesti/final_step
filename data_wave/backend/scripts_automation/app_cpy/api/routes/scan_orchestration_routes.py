...
async def monitor_orchestration_progress(orchestration_id: str, username: str):
    """Background task to monitor orchestration progress using enterprise monitoring services"""
    try:
        ...
        logger.info(f"Starting enterprise progress monitoring for orchestration {orchestration_id}")
        ...
        orchestration = await orchestration_service.get_orchestration_details(orchestration_id)
        ...
        while True:
            try:
                ...
                if current_status != last_status:
                    logger.info(f"Orchestration {orchestration_id} status changed: {last_status} -> {current_status}")
                    last_status = current_status
                    
                    # Send status update notification
                    await notification_service.send_orchestration_status_update(
                        orchestration_id=orchestration_id,
                    ...
                    await orchestration_service.log_status_change(
                        orchestration_id=orchestration_id,
                ...
                if current_status in ['completed', 'failed', 'cancelled']:
                    logger.info(f"Orchestration {orchestration_id} finished with status: {current_status}")
                    
                    # Send completion notification
                    await notification_service.send_orchestration_completion_notification(
                        orchestration_id=orchestration_id,
                        status=current_status,
                        username=username,
                        duration=datetime.utcnow() - monitoring_start
                    )
                    
                    # Update orchestration completion metrics
                    await monitoring_service.update_orchestration_metrics(
                        orchestration_id=orchestration_id,
                        status=current_status,
                        duration=datetime.utcnow() - monitoring_start
                    )
                ...
                performance_metrics = await monitoring_service.get_orchestration_performance(orchestration_id)
                if performance_metrics.get('performance_score', 100) < 70:
                    logger.warning(f"Orchestration {orchestration_id} performance degraded: {performance_metrics.get('performance_score')}")
                ...
                logger.error(f"Error monitoring orchestration {orchestration_id}: {e}")
                await asyncio.sleep(10)  # Wait longer on error
        
        logger.info(f"Enterprise progress monitoring completed for orchestration {orchestration_id}")
        
    except Exception as e:
        logger.error(f"Error in enterprise orchestration progress monitoring: {e}")

async def monitor_bulk_execution_progress(bulk_execution_id: str, orchestration_ids: List[str], username: str):
    """Background task to monitor bulk execution progress using enterprise monitoring services"""
    try:
        from app.services.scan_orchestration_service import ScanOrchestrationService
        from app.services.advanced_monitoring_service import AdvancedMonitoringService
        from app.services.enterprise_integration_service import EnterpriseIntegrationService
        ...
        logger.info(f"Starting enterprise bulk execution monitoring for {bulk_execution_id}")
        ...
        # Get bulk execution details
        bulk_execution = await orchestration_service.get_bulk_execution_details(bulk_execution_id)
        if not bulk_execution:
            logger.warning(f"Bulk execution {bulk_execution_id} not found")
            return
        
        # Start comprehensive monitoring
        monitoring_start = datetime.utcnow()
        completed_orchestrations = set()
        failed_orchestrations = set()
        
        while True:
            try:
                # Get overall bulk execution status
                overall_status = await orchestration_service.get_bulk_execution_status(bulk_execution_id)
                
                # Check individual orchestration statuses
                for orchestration_id in orchestration_ids:
                    if orchestration_id in completed_orchestrations or orchestration_id in failed_orchestrations:
                        continue
                    
                    orchestration_status = await orchestration_service.get_orchestration_status(orchestration_id)
                    
                    if orchestration_status == 'completed':
                        completed_orchestrations.add(orchestration_id)
                        logger.info(f"Orchestration {orchestration_id} completed in bulk execution {bulk_execution_id}")
                        
                    elif orchestration_status == 'failed':
                        failed_orchestrations.add(orchestration_id)
                        logger.warning(f"Orchestration {orchestration_id} failed in bulk execution {bulk_execution_id}")
                        
                        # Send failure notification
                        await notification_service.send_orchestration_failure_notification(
                            orchestration_id=orchestration_id,
                            bulk_execution_id=bulk_execution_id,
                            username=username
                        )
                ...
                # Calculate progress
                total_orchestrations = len(orchestration_ids)
                completed_count = len(completed_orchestrations)
                failed_count = len(failed_orchestrations)
                progress_percentage = (completed_count + failed_count) / total_orchestrations * 100
                
                # Update bulk execution progress
                await orchestration_service.update_bulk_execution_progress(
                    bulk_execution_id=bulk_execution_id,
                ...
                # Check if all orchestrations are complete
                if completed_count + failed_count == total_orchestrations:
                    logger.info(f"Bulk execution {bulk_execution_id} completed")
                    
                    # Send completion notification
                    await notification_service.send_bulk_execution_completion_notification(
                        bulk_execution_id=bulk_execution_id,
                        completed_count=completed_count,
                        failed_count=failed_count,
                        username=username,
                    ...
                    # Update bulk execution metrics
                    await monitoring_service.update_bulk_execution_metrics(
                        bulk_execution_id=bulk_execution_id,
                        completed_count=completed_count,
                        failed_count=failed_count,
                        duration=datetime.utcnow() - monitoring_start
                ...
                if progress_percentage % 25 == 0:  # Every 25% progress
                    await notification_service.send_progress_update_notification(
                        bulk_execution_id=bulk_execution_id,
                        progress_percentage=progress_percentage,
                        completed_count=completed_count,
                        failed_count=failed_count,
                ...
                logger.error(f"Error monitoring bulk execution {bulk_execution_id}: {e}")
                await asyncio.sleep(15)  # Wait longer on error
        
        logger.info(f"Enterprise bulk execution monitoring completed for {bulk_execution_id}")
        
    except Exception as e:
        logger.error(f"Error in enterprise bulk execution progress monitoring: {e}")
