...
async def endpoint_concurrency_middleware(request: Request, call_next):
    ...
    # Mark schema discovery class to extend DB semaphore timeout downstream
    is_discovery = path.startswith("/data-discovery")
    set_token = None
    if is_discovery:
        try:
            from app import db_session as _db
            set_token = _db.set_request_class("schema_discovery")
        except Exception:
            set_token = None
    ...
    try:
        ...
        if is_discovery:
            try:
                from app import db_session as _db
                _db.reset_request_class(set_token)
            except Exception:
                pass
