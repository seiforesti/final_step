...
class RuleValidationEngine:
    ...
    async def _validate_sql_syntax(
        ...
        try:
            ...
            # Enterprise SQL parsing validation
            try:
                import sqlparse  # type: ignore
                parsed = sqlparse.parse(sql_expression)
                if not parsed:
                    result["valid"] = False
                    result["errors"].append("SQL could not be parsed")
                else:
                    # Heuristics: ensure a top-level statement and known DML/DDL
                    stmt = parsed[0]
                    tokens = [t for t in stmt.tokens if not t.is_whitespace]
                    text_up = sql_expression.upper()
                    if not any(k in text_up for k in ("SELECT", "INSERT", "UPDATE", "DELETE", "WITH", "CREATE", "ALTER")):
                        result["warnings"].append("SQL expression may not be a DML/DDL statement")
                        result["syntax_score"] *= 0.85
            except Exception:
    ...
    async def _validate_json_syntax(self, json_expression: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Enterprise JSON schema validation
            if isinstance(parsed_json, dict):
                try:
                    from jsonschema import Draft7Validator  # type: ignore
                    v = Draft7Validator(parsed_json)
                    # Run a no-op validate with empty instance just to compile schema
                    list(v.iter_errors({}))
                except Exception as e:
                    result["warnings"].append(f"Schema validation warning: {str(e)}")
                    result["syntax_score"] *= 0.85
