...
class ScanOrchestrationService:
    ...
    def _estimate_wait_time(self) -> int:
        ...
        # Enterprise estimation using utilization and historical durations
        try:
            active_count = len(self.active_orchestrations)
            if active_count == 0:
                return 0
            # Compute mean remaining time from active contexts if present
            remaining: List[int] = []
            for ctx in self.active_orchestrations.values():
                rem = int(ctx.get("estimated_remaining_minutes", 0))
                if rem > 0:
                    remaining.append(rem)
            avg_remaining = int(sum(remaining) / len(remaining)) if remaining else 15
            # Adjust by queue depth and priority weight
            queue_depth = len(self.orchestration_queue)
            penalty = min(60, queue_depth * 2)
            estimate = avg_remaining + penalty
            return min(240, max(3, estimate))
        except Exception:
            return 15
