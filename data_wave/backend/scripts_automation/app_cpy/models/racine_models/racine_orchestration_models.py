...
class RacineCrossGroupIntegration(SQLModel, table=True):
    ...
    """
    __tablename__ = "racine_cross_group_integration_orchestration"
...
class RacineIntegrationStatus(SQLModel, table=True):
    ...
    id: str = Field(default_factory=lambda: str(uuid.uuid4()), primary_key=True)
    integration_id: str = Field(foreign_key="racine_cross_group_integration_orchestration.id", index=True)
