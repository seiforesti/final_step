...
_request_class: ContextVar[str] = ContextVar("_request_class", default="default")


def set_request_class(request_class: str):
    """Set the current request class in a ContextVar. Returns a token for reset."""
    try:
        return _request_class.set(request_class)
    except Exception:
        return None

def reset_request_class(token) -> None:
    try:
...
def _create_engine(database_url: str, *, pool_size_override: int | None = None, max_overflow_override: int | None = None, pool_timeout_override: int | None = None) -> Engine:
    ...
    try:
        # With PgBouncer (transaction pooling), avoid long-lived transactions and rollbacks on close
        if use_pgbouncer:
...
def init_db(max_retries: int = 10, backoff_seconds: float = 1.0) -> None:
    ...
    while True:
        try:
            ...
            # Log connection pool status (guard for PgBouncer/NullPool)
            try:
                database_url = os.getenv("DATABASE_URL", "")
                is_using_pgbouncer = "pgbouncer" in database_url.lower() or "6432" in database_url
                if is_using_pgbouncer:
                    logger.info("Database connection pool status: managed_by_pgbouncer")
                else:
                    logger.info(
                        f"Database connection pool status: size={engine.pool.size()}, checked_in={engine.pool.checkedin()}, checked_out={engine.pool.checkedout()}, overflow={engine.pool.overflow()}"
                    )
            except Exception:
                logger.info("Database connection pool status: unavailable (non-standard pool)")
...
async def init_async_db(max_retries: int = 10, backoff_seconds: float = 1.0) -> None:
    ...
    try:
        ...
        logger.warning(f"Model import warning during init_async_db: {type(import_error).__name__}: {import_error}")
...
def get_db() -> Generator[Session, None, None]:
    ...
    try:
        from app.db_config import DB_CONFIG
        base_timeout = float(DB_CONFIG["semaphore_timeout"]) if isinstance(DB_CONFIG["semaphore_timeout"], (int, float)) else DB_CONFIG["semaphore_timeout"]
        semaphore_timeout = float(base_timeout)
    except Exception:
        semaphore_timeout = float(os.getenv("DB_SEMAPHORE_TIMEOUT", "5.0"))
    # Give schema discovery requests a longer wait to avoid 429/503 churn while serialized by middleware
    try:
        if _request_class.get("default") == "schema_discovery":
            # Cap at 30s to avoid hanging
            semaphore_timeout = max(semaphore_timeout, 30.0)
    except Exception:
        pass
...
async def get_async_session(priority=None) -> Generator[AsyncSession, None, None]:
    """FastAPI async dependency yielding an AsyncSession. Priority is accepted and ignored."""
...
@contextmanager
def get_sync_db_session(priority=None) -> Generator[Session, None, None]:
